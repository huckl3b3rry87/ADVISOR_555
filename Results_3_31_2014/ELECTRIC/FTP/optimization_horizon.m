clear
clc
load small_electric_FTP_results
%%% FIX THIS ONE _ GET RID OF THE 7 in itteration number wher the sim
%%% starts working
figure(1);
mpg = small_electric_FTP_optimization.GLOBAL.f_min_hist.c(:,2);
iter = small_electric_FTP_optimization.GLOBAL.f_min_hist.iter;
plot(iter,mpg,'linewidth',8)
xlabel('Iterations')
ylabel('MPG'),grid
%title('Small electric FTP')
...
fprintf('\nMotor')
trq_m = small_electric_FTP_optimization.GLOBAL.f_min_hist.x(:,1);
spd_m = small_electric_FTP_optimization.GLOBAL.f_min_hist.x(:,2);
First = 58*trq_m(7)*spd_m(7)
Last = 58*trq_m(length(trq_m))*spd_m(length(spd_m))
Increase = (Last-First)/Last*100

fprintf('\nBattery Module')
batt = small_electric_FTP_optimization.GLOBAL.f_min_hist.x(:,3);
First = batt(7)
Last = batt(length(batt))
Increase = (Last-First)/Last*100

% Other Component parameters
fd = small_electric_FTP_optimization.GLOBAL.f_min_hist.x(:,5);
cap_scale = small_electric_FTP_optimization.GLOBAL.f_min_hist.x(:,4);

% Initial Component Parameters
mc_trq_scale = trq_m(7);
mc_spd_scale = spd_m(7);
ess_module_num = batt(7);
fd_ratio = fd(7);
cap_scale = cap_scale(7);
