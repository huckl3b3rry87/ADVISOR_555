% ADVISOR data file:  ESS_NICAD102.m    where 102 is c/5-rate capacity
%
% Data source:  
...
% FILE ID INFO
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ess_description='Saft STM 5-100 6-V battery, tested at UC-Davis';
...
disp(['Data loaded: ESS_NICAD102 - ',ess_description])


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SOC RANGE over which data is defined
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ess_soc=[0.0667 0.167 0.33 0.5 0.667 0.833 0.967];  % (--)
...
% LIMITS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ess_min_volts=5.9;%ess_voc(1)-ess_max_ah_cap/5*ess_r_dis(1);
ess_max_volts=6.9;%ess_voc(end)+ess_max_ah_cap/5*ess_r_chg(end);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% OTHER DATA
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ess_module_mass=14;  % (kg), mass of a single module
...
ess_mod_cp=770;                            % J/kgK  ave heat capacity of module (estimated for NiCd)
ess_set_tmp=35;                            % C      thermostat temp of module when cooling fan comes on
ess_area_scale=1.3*(ess_module_mass/11)^0.7;   % --     if module dimensions are unknown, assume rectang shape and scale vs PB25
