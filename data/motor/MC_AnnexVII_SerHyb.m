% ADVISOR Data file:  MC_AnnexVII_SerHyb.m
%
% Data source:
% 
%
...
% FILE ID INFO
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
mc_description='EM for Annex VII HD series hybrid vehicle (eff is constant) no inertia';
...
disp(['Data loaded: MC_AnnexVII_SerHyb - ',mc_description]);


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SPEED & TORQUE RANGES over which data is defined
max_trq=1519;%[Nm]
max_speed=5232; %[1/min]
base_speed=1048; %[1/min]
mc_overtrq_factor=1;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% (rad/s), speed range of the motor
mc_map_spd=[0:base_speed/5:base_speed ((base_speed+(max_speed-base_speed)/10):(max_speed-base_speed)/10:max_speed)]*(2*pi/60);
% Conversion from RPM to rad/s 

% (N*m), torque range of the motor
mc_map_trq=(0:max_trq/10:max_trq);
...
mc_eff_map=ones(length(mc_map_spd),length(mc_map_trq))*0.9;
...
mc_max_trq=[];
mc_map_spd=[0:base_speed/5:base_speed ((base_speed+(max_speed-base_speed)/10):(max_speed-base_speed)/10:max_speed)]*(2*pi/60);
x=find(mc_map_spd<=(base_speed/60*(2*pi)));
mc_max_trq(x)=max_trq; % (N*m)
mc_max_trq=[mc_max_trq (base_speed/60*(2*pi)*max_trq)./mc_map_spd((x(end)+1):end)];

mc_max_gen_trq=-mc_max_trq; % (N*m), estimate
...
mc_max_crrnt=inf; % (A), maximum current allowed by the controller and motor
mc_min_volts=1; % (V), minimum voltage allowed by the controller and motor
...
% OTHER DATA
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
mc_inertia=0;  % (kg*m^2), rotor's rotational inertia
mc_mass=10;  % (kg), mass of motor !! can't be zero!! because of temperature calculations
...
clear max_trq max_speed base_speed;
