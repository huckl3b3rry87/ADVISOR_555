% ADVISOR data file:  PTC_PAR_CD.m
%
% Data source:
%
% Data confirmation:
...
% and engine controls, for a charge depleting parallel hybrid using a 5-spd gearbox.
% 
% Created on: 12/17/99
...
% FILE ID INFO
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ptc_description='5-spd parallel charge depleting hybrid';
...
disp(['Data loaded: PTC_PAR_CD - ',ptc_description])
...
gb_gear1_upshift_spd=min(fc_map_spd)+[0.35 0.45 0.98]*(max(fc_map_spd)-min(fc_map_spd))*fc_spd_scale;  % (rad/s)
...
cs_lo_soc=0.3;  % (--), lowest desired battery state of charge

% vehicle speed below which vehicle operates as ZEV
% at low SOC
cs_electric_launch_spd_lo=8;  % (m/s)
...
% REVISION HISTORY
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 12/17/99:ss file created from ptc_par
% 2/22/00:tm revised parameter settings to represent a charge depleting hybrid
