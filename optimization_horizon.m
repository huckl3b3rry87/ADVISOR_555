clear
clc
load small_electric___results

format short

figure(1);
mpg = small_electric___optimization.GLOBAL.f_min_hist.c(:,2);
iter = small_electric___optimization.GLOBAL.f_min_hist.iter;
plot(iter,mpg,'linewidth',8)
xlabel('Iterations')
ylabel('MPG'),grid
%title('Small electric _')
...
fprintf('\nMotor')
trq_m = small_electric___optimization.GLOBAL.f_min_hist.x(:,1);
spd_m = small_electric___optimization.GLOBAL.f_min_hist.x(:,2);
First = 58*trq_m(1)*spd_m(1)
Last = 58*trq_m(length(trq_m))*spd_m(length(spd_m))
Increase = (Last-First)/Last*100

fprintf('\nBattery Module')
batt = small_electric___optimization.GLOBAL.f_min_hist.x(:,3);
First = batt(1)
Last = batt(length(batt))
Increase = (Last-First)/Last*100

% Other Component parameters
fd = small_electric___optimization.GLOBAL.f_min_hist.x(:,4);
cap_scale = small_electric___optimization.GLOBAL.f_min_hist.x(:,5);
