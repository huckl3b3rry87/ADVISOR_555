%function logFlag=advSACompMaskInit(generalName,logPick,initFileName,modCell,DEBUG)
% advSACompMaskInit.m
...
%         end
        ...
        if ~isempty(modCell)
            for j=1:length(modCell(:,1))
                %eval([modCell{j,1},'=',modCell{j,2},';']);
                %assignin('caller',modCell{j,1},modCell{j,2});
...
%         set_param([gcb,'/constTag'],'GotoTag',[generalName,'Const'])
%         set_param([gcb,'/constTag'],'TagVisibility','global')        
    end
catch
    if DEBUG
        disp(lasterr)
