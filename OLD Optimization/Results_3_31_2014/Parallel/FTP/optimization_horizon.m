clear
clc
load small_parallel_FTP_results

figure(1);
mpg = small_parallel_FTP_optimization.GLOBAL.f_min_hist.c(:,3);
iter = small_parallel_FTP_optimization.GLOBAL.f_min_hist.iter;
plot(iter,mpg,'linewidth',8)
xlabel('Iterations')
ylabel('MPG'),grid
%title('Small Parallel FTP')
set(gca,'FontSize',15,'fontWeight','bold')
set(findall(gcf,'type','text'),'FontSize',22,'fontWeight','bold')

figure(2);
CO = small_parallel_FTP_optimization.GLOBAL.f_min_hist.c(:,4);
iter = small_parallel_FTP_optimization.GLOBAL.f_min_hist.iter;
...
figure(3);
HC = small_parallel_FTP_optimization.GLOBAL.f_min_hist.c(:,5);
iter = small_parallel_FTP_optimization.GLOBAL.f_min_hist.iter;
...
figure(4);
NOx = small_parallel_FTP_optimization.GLOBAL.f_min_hist.c(:,6);
iter = small_parallel_FTP_optimization.GLOBAL.f_min_hist.iter;
...
fprintf('\nICE')
trq = small_parallel_FTP_optimization.GLOBAL.f_min_hist.x(:,1);
First = 41*trq(1)
Last = 41*trq(length(trq))
Increase = (Last-First)/Last*100

fprintf('\nMotor')
trq_m = small_parallel_FTP_optimization.GLOBAL.f_min_hist.x(:,2);
First = 58*trq_m(1)
Last = 58*trq_m(length(trq_m))
Increase = (Last-First)/Last*100

fprintf('\nBattery Module')
batt = small_parallel_FTP_optimization.GLOBAL.f_min_hist.x(:,3);
First = batt(1)
