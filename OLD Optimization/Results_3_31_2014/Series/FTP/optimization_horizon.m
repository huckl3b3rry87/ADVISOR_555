clc
clear
load small_series_FTP_results

figure(1);
mpg = small_series_FTP_optimization.GLOBAL.f_min_hist.c(:,3);
iter = small_series_FTP_optimization.GLOBAL.f_min_hist.iter;
plot(iter,mpg,'linewidth',8)
xlabel('Iterations')
ylabel('MPG'),grid
%title('Small series FTP')
set(gca,'FontSize',15,'fontWeight','bold')
set(findall(gcf,'type','text'),'FontSize',22,'fontWeight','bold')

figure(2);
CO = small_series_FTP_optimization.GLOBAL.f_min_hist.c(:,4);
iter = small_series_FTP_optimization.GLOBAL.f_min_hist.iter;
...
figure(3);
HC = small_series_FTP_optimization.GLOBAL.f_min_hist.c(:,5);
iter = small_series_FTP_optimization.GLOBAL.f_min_hist.iter;
...
figure(4);
NOx = small_series_FTP_optimization.GLOBAL.f_min_hist.c(:,6);
iter = small_series_FTP_optimization.GLOBAL.f_min_hist.iter;
...
fprintf('\nICE')
trq = small_series_FTP_optimization.GLOBAL.f_min_hist.x(:,1);
spd = small_series_FTP_optimization.GLOBAL.f_min_hist.x(:,2);
First = 41*trq(1)*spd(1)
Last = 41*trq(length(trq))*spd(length(spd))
Increase = (Last-First)/Last*100

fprintf('\nMotor')
trq_m = small_series_FTP_optimization.GLOBAL.f_min_hist.x(:,3);
First = 58*trq_m(1)
Last = 58*trq_m(length(trq_m))
Increase = (Last-First)/Last*100

fprintf('\nGenerator')
trq_g = small_series_FTP_optimization.GLOBAL.f_min_hist.x(:,5);
First = 32*trq_g(1)
Last = 32*trq_g(length(trq_g))
Increase = (Last-First)/Last*100

fprintf('\nBattery Module')
batt = small_series_FTP_optimization.GLOBAL.f_min_hist.x(:,4);
First = batt(1)
Last = batt(length(batt))
Increase = (Last-First)/Last*100

% Other Component parameters
fd = small_series_FTP_optimization.GLOBAL.f_min_hist.x(:,6);
cap_scale = small_series_FTP_optimization.GLOBAL.f_min_hist.x(:,7);
...
% Control Parameters
hi_soc = small_series_FTP_optimization.GLOBAL.f_min_hist.x(:,8);
lo_soc = small_series_FTP_optimization.GLOBAL.f_min_hist.x(:,9);
max_pwr = small_series_FTP_optimization.GLOBAL.f_min_hist.x(:,10);
min_pwr = small_series_FTP_optimization.GLOBAL.f_min_hist.x(:,11);
