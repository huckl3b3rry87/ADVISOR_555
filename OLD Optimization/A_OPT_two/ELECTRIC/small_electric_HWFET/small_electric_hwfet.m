...
resp_names={'MPGGE_HWFET_small_electric'};
...
x_L=[0.5*mc_trq_scale,          0.5*mc_spd_scale,             round(0.5*ess_module_num),  0.5,   0.5*fd_ratio]';
x_U=[1.5*mc_trq_scale,          1.5*mc_spd_scale,             round(1.5*ess_module_num),  1.5,  1.5*fd_ratio]';
...
MaxIter=50;
GLOBAL.epsilon=1e-6;
prev_results_filename='small_electric_HWFET_results';

if cont_bool==1
   eval(['load(''',prev_results_filename,''')']) 
   GLOBAL = small_electric_hwfet_optimization.GLOBAL;
...
small_electric_hwfet_optimization = gclSolve(p_f, p_c, x_L, x_U, A, b_L, b_U, c_L, c_U, I, GLOBAL, PriLev, plot_info, dv_names, resp_names, con_names);

% save the results
eval(['save(''',prev_results_filename,''',','''small_electric_hwfet_optimization'');']) 

% save the vehicle
input.save.filename='small_electric_hwfet';
[a,b]=adv_no_gui('save_vehicle',input);

% plot the optimization results
PlotOptimResults(small_electric_hwfet_optimization.GLOBAL.f_min_hist, plot_info)
