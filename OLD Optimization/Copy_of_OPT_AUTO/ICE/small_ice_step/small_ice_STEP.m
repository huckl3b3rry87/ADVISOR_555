...
dv_names={'fc_spd_scale','fc_trq_scale','fc_pwr_scale'};
resp_names={'MPGGE_STEP_small_ice'};
...
MaxIter=2;
GLOBAL.epsilon=1e-4;
prev_results_filename='small_ice_STEP_results';

if cont_bool==1
   eval(['load(''',prev_results_filename,''')']) 
   GLOBAL = small_ice_STEP_optimization.GLOBAL;
...
small_ice_STEP_optimization = gclSolve(p_f, p_c, x_L, x_U, A, b_L, b_U, c_L, c_U, I, GLOBAL, PriLev, plot_info, dv_names, resp_names, con_names);

% save the results
eval(['save(''',prev_results_filename,''',','''small_ice_STEP_optimization'');']) 

% save the vehicle
input.save.filename='small_ice_STEP';
[error_code,b]=adv_no_gui('save_vehicle',input);

% plot the optimization results
PlotOptimResults(small_ice_STEP_optimization.GLOBAL.f_min_hist, plot_info)
